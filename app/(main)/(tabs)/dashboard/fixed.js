...
export default function FixedIssuesScreen() {
  ...
  const getStatusColor = (status) => {
    ...
    if (s === 'COMPLETED' || s === 'RESOLVED') return { bg: isDark ? 'rgba(16, 185, 129, 0.15)' : '#dcfce7', text: '#059669' };
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      ...
      >
        <View style={[styles.healthBar, { backgroundColor: '#10a37f' }]} />
        ...
        <View style={styles.userRow}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: statusColor.bg }]}>
            <Text style={[styles.statusText, { color: statusColor.text }]}>{item.status || 'COMPLETED'}</Text>
          </View>
        </View>
  ...
  if (loading && issues.length === 0 && !refreshing) return <Loader message="Loading resolved issues..." />;
    ...
    <SafeAreaView edges={['top']} style={[styles.container, { backgroundColor: bgColor }]}>
      ...
      <View style={styles.resultsHeader}>
        <Text style={[styles.resultsCount, { color: theme.textSecondary }]}>
          RESOLVED RECORDS ({filteredIssues.length})
        </Text>
      </View>
      ...
      <FullScreenSpinner visible={refreshing} message="Updating Resolved Issues..." color={primaryBlue} />
